...
#-----------------------------------------------------------------------------
# Read in DHS data
all_data <- readRDS("data/DHS/brazil_fathers.RDS")
all_data$father_status <- rep(0, length(all_data$id_household))
...
for (y in 1:3){
  ...
  full <- father_data[which(father_data$age_father_now > 0 & father_data$age_mother_now >0) ,]
...
saveRDS(df_fertility, file = "data/Brazil/male_fertility.RDS")
