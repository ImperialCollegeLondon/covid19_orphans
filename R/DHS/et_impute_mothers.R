...
for (y in 1:3){
  # Read in DHS data
  if (y == 1){
    all_data <- readRDS("data/DHS/et_mothers_16.RDS")
  } else if (y == 2){
    all_data <- readRDS("data/DHS/et_mothers_11.RDS")
  } else {
    all_data <- readRDS("data/DHS/et_mothers_05.RDS")
  }
...
names(df_fertility) <- paste0("y", 2016 - 0:15)
df_fertility$ages <- factor(exposure_years$label, levels = c("under 15", labels))

saveRDS(df_fertility, file = "data/Ethiopia/female_fertility.RDS")
...
p <- ggplot(df_fertility) + 
  ...
  scale_color_manual(values = c("2016" = "black", "2011" = "blue", "2005" = "red")) + 
  theme_bw()
print(p)