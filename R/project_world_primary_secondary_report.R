...
country_samples <- as.data.frame(estimates_orphans)
...
for (c in study_countries){
  country_samples[which(country_samples$country == c),1:1000] = joined$final_orphans[joined$country == c]
}
...
saveRDS(region_data, "data/region_estimates_ps.RDS")
