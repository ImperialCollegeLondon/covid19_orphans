...
add_child_mortality = function(is_child_mortality_needed, country){
  children = read.csv(paste0('data/', country, '/child_raw_m.csv'))
  ...
  if (is_child_mortality_needed){
    ...
    write_csv(file = paste0('data/', country, '/child_all_list_m.csv'), plot_c_and_m)
  } else{
  ...
  child_and_m = read.csv(paste0('data/', country, '/child_all_m.csv'))
  ddf = as.data.frame(apply(child_and_m, 1, sum))
  names(ddf) = 'children'
  ddf$gender = 'male'
  ddf$age = 1:100
  write_csv(file = paste0('data/', country, '/children_m.csv'),ddf)
}
# 
# # Multiple countries
...
process_children_mortality_england_wales = function(){
  ...
  #download.file(url, 'data/children/mortality_england_wales.xls')
  data = readxl::read_xls('data/UK/mortality_england_wales.xls', sheet = 5)
...
process_infant_mortality_england_wales = function(){
  ...
  #download.file(url, 'data/UK/infant_england_wales.csv')
  data = read.csv('data/UK/infant_england_wales.csv')
...
process_child_mortality_russia = function(){
  data_04 = read.csv('data/Russia/russian_federation_0-4.csv')
  data_04 = data_04[,c(1,4:5)]
  setnames(data_04, 1:3, c('country', 'year', 'mortality'))
  data_04$age = '0-4'  
  data_59 = read.csv('data/Russia/russian_federation_5-9.csv')
  data_59 = data_59[,c(1,4:5)]
  setnames(data_59, 1:3, c('country', 'year', 'mortality'))
  data_59$age = '5-9'  
  data_1014 = read.csv('data/Russia/russian_federation_10-14.csv')
  data_1014 = data_1014[,c(1,4:5)]
  setnames(data_1014, 1:3, c('country', 'year', 'mortality'))
  data_1014$age = '10-14'  
  data_1519 = read.csv('data/Russia/russian_federation_15-19.csv')
  ...
  write_csv(data,path = paste0('data/children/mortality_rate_all_russain_federation.csv'))
