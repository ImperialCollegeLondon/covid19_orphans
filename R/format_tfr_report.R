...
# Remove France and Denmark
deaths_country = deaths_country[! deaths_country$Country_Region %in% c("France"),] 
fra = deaths[deaths$Country_Region == "France",]
fra$Province_State[fra$Province_State == ""] = "France"
fra = select(fra, Province_State, Deaths)
names(fra) = c("Country_Region", "total_deaths")
fra = fra[which(fra$Country_Region %in% c("France", "French Guiana")),]

deaths_country = rbind(deaths_country, fra)
...
saveRDS(joined, "data/tfr_covariates_report.RDS")