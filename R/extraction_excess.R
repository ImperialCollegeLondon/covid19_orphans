...
process_euro_excess_new = function(){
  list_excel = readxl::excel_sheets('data/Six_euro_Mar_2021/DEMO_R_MWK_05_six_countries_2021.xlsx')
  list_excel = list_excel[3:length(list_excel)]
  x <- lapply(list_excel, function(X) readxl::read_excel('data/Six_euro_Mar_2021/DEMO_R_MWK_05_six_countries_2021.xlsx', sheet = X))
  names(x) <- list_excel
  ...
  write.csv(age_summary, file = paste0("data/Six_euro_Mar_2021/euro_excess.csv"), row.names = FALSE)
}

# Process Argentina - covid only, updated to 31st dec
process_argentina_covid19 = function(){
  data = read.csv('data/Argentina/Argentina - Deaths by age and sex (rates per 100,000)-mar.csv')
  ...
  write_csv(file = 'data/Argentina/covid19_deaths.csv', d_merge)
  d_merge
}
...
process_brazil = function(age_range){
  ...
  write_csv(path = paste0("data/Brazil/","brazil_summary.csv"),tmp)
  
  d_merge = tmp
  d_merge$date = as.Date(d_merge$date)
  d_merge = d_merge[which(d_merge$date <= as.Date("2021-03-31")),]
  d_merge = d_merge %>% 
  ...
  d_merge$date = rep('2021-03-31', length(d_merge$date))
  d_merge$rate = d_merge$deaths/sum(d_merge$deaths)
  d_merge$deaths = round(d_merge$rate * total)
  setnames(d_merge, 'deaths', 'COVID19_deaths')
  write_csv(d_merge %>% select(-rate), path = 'data/Brazil/covid19_deaths.csv')
}

#Colombia
process_colombia_covid19 = function(){
  data = readxl::read_xlsx('data/Colombia/COVID19_Colombia_by_sex_and_age.xlsx')
  ...
  write_csv(data, path = 'data/Colombia/covid19_deaths_all.csv')
}
...
process_england_wales = function(){ # Just use excess
  ...
  #download.file(url_previous, 'data/UK/england_wales_pre_deaths.xlsx')
  data = readxl::read_xlsx('data/UK/england_wales_pre_deaths.xlsx', sheet = 2)
  ...
  url = 'https://download.ons.gov.uk/downloads/datasets/weekly-deaths-age-sex/editions/covid-19/versions/27.csv'
  #download.file(url, 'data/UK/england_wales_deaths-v27.csv')
  data2 = read.csv('data/UK/england_wales_deaths-v27.csv')
  
  week_split <- function(x){
    return(as.double(strsplit(x, "-")[[1]][2]))
  }
  data2$week = sapply(as.character(data2$week.number), week_split)
  data2 = data2[which(!(data2$calendar.years == "2021" & data2$week > 13)),]  #week 13 includes 31st Mar
  data2$week <- NULL
...
process_france = function(){
  tmp = readxl::read_xlsx('data/France/Deaths-Age-Sex_Covid-19_France_15-04.xlsx', sheet = 2)
  data = tmp[7:16, c(1,c(113,115,118))]
  ...
  excess_death = excess_death %>%
    filter((year == 2020 & week >= 10)  | (year == 2021 & week <= 13)) %>%
    group_by(age, gender) %>%
    summarise("excess_deaths" = sum(France))
  
  d_merge = left_join(data, excess_death, by = c('gender', 'age' = 'age'))
  write_csv(path = paste0("data/France/","france_all.csv"),d_merge)
  ...
  write_csv(d_merge, path = 'data/France/france_all.csv')
}

# Germany
process_germany = function(){
  tmp = readxl::read_xlsx('data/Germany/Deaths-Age-Sex_Covid-19_Germany_15-04.xlsx', sheet = 2)
  data = tmp[7:16, c(1,c(22,24,27))]
  data = data.frame(data)
  names(data) = c('age', 'male', 'female', 'both')
  ...
  write_csv(data, path = 'data/Germany/covid19_deaths.csv')
}

# India
process_india_covid = function(){
  total_deaths = 162960 # 31st March 157194 # on 28th feb from https://www.covid19india.org
  demog <- read_excel("data/India/jghs-2-e17-i002.xlsx")
  ...
  write_csv(d_summary %>% arrange(age, sex), path = 'data/India/all_covid_deaths.csv')
  ...
  write_csv(d_summary, path = 'data/India/covid_deaths.csv')
}
...
process_italy = function(){
  tmp = readxl::read_xlsx('data/Italy/Deaths-Age-Sex_Covid-19_Italy_08-04.xlsx', sheet = 2)
  ...
  excess_death = excess_death %>%
    filter((week >= 10 & year == "2020") | (week <= 8 & year == "2021")) %>%
    group_by(age, gender) %>%
    summarise("excess_deaths" = sum(excess_death))
  
  d_merge = left_join(data, excess_death, by = c('gender', 'age' = 'age'))
  write_csv(path = paste0("data/Italy/","italy_all.csv"),d_merge)
  ...
  d_merge$deaths = ifelse(d_merge$covid_deaths > d_merge$excess_deaths, d_merge$covid_deaths, d_merge$excess_deaths)
  d_merge = d_merge %>% select(age, gender, covid_deaths, excess_deaths, deaths)
  write_csv(d_merge, path = 'data/Italy/italy_all.csv')
}
...
process_kenya_covid19 = function(){
  ...
  write_csv(data, path = 'data/Kenya/covid19_deaths.csv')
}

# Malawi
process_malawi = function(){
  data = read_xls('data/Malawi/malawi.xls')
  ...
  write_csv(data, file = 'data/Malawi/covid19_deaths.csv')
}

# Mexico
process_mexico_covid19 = function(){
  data = read.csv('data/Mexico/Mexico - Deaths by age and sex (rates per 100,000)-mar.csv', fileEncoding="latin1")
  ...
  write_csv(data_combine %>% select(age, gender, deaths), path = 'data/Mexico/covid19_deaths.csv')
}

# Nigeria
process_nigeria_covid19 = function(){
  d_merge = read.csv('data/Nigeria/covid19_deaths_raw_mar.csv')
  ...
  d_merge$date = '2021-03-31'
  d_merge$rate = d_merge$deaths/sum(d_merge$deaths)
  d_merge$deaths = round(d_merge$rate * total)
  write_csv(d_merge %>% select(-rate), path = 'data/Nigeria/covid19_deaths.csv')
}
...
process_peru_covid19 = function(){
  ...
  write_csv(d_merge %>% select(-rate), path = 'data/Peru/covid19_deaths.csv')
}

# Philippines 

process_philippines = function(){
  data_1 <- read_csv("data/Philippines/DOH COVID Data Drop_ 20210509 - 04 Case Information 2021.csv")
  data_2 <- read_csv("data/Philippines/DOH COVID Data Drop_ 20210509 - 04 Case Information 2020.csv")
  data <- rbind(data_1, data_2)
  data$DateDied <- as.Date(data$DateDied)
  
  data_sub <- data[which(data$RemovalType == 'DIED'),]
  data_sub <- data_sub[which(data_sub$DateDied <= as.Date('2021-03-31')),]
  ...
  if (total > sum(d$deaths)){
    ...
    write_csv(d %>% select(-rate), path = 'data/Philippines/covid19_deaths.csv')
  } else {
    
    setnames(d, 'deaths', 'COVID19_deaths')
    write_csv(d, path = 'data/Philippines/covid19_deaths.csv')
  }
}
...
process_poland_covid19_and_excess = function(){
  ...
  write_csv(data, path = 'data/Poland/covid19_deaths.csv')
  
  data$COVID19_deaths = as.numeric(data$COVID19_deaths)
  write_csv(path = paste0("data/Poland/","poland_deaths.csv"),data)  
  ...
  write_csv(data, path = 'data/Poland/poland_all.csv')
}
...
process_russia_excess = function(){
  ...
  total_excess = 381555
  demog <- read_excel("data/Russia/demo24.xls")
  demog_2019 <- demog[33,]
  ...
  data_pop = readxl::read_xlsx('data/Russia/pop.xlsx', sheet = 2)
  ...
  write_csv(d_summary %>% arrange(age, sex), path = 'data/Russia/all_excess_deaths.csv')
  ...
  write_csv(d_summary, path = 'data/Russia/excess_deaths.csv')
}


# Spain
process_spain = function(){
  tmp = readxl::read_xlsx('data/Spain/Deaths-Age-Sex_Covid-19_Spain_08-04.xlsx', sheet = 6)
  data = tmp[7:15, c(1,c(50,52,55))]
  data = data.frame(data)
  names(data) = c('age', 'male', 'female', 'both')
  data$male = as.numeric(data$male)
  data$female = as.numeric(data$female)
  ...
  d_merge = left_join(data, excess_death, by = c('gender', 'age' = 'age'))
  write_csv(path = paste0("data/Spain/","spain_all.csv"),d_merge)
  ...
  d_merge$deaths = ifelse(d_merge$covid_deaths > d_merge$excess_deaths, d_merge$covid_deaths, d_merge$excess_deaths)
  d_merge = d_merge %>% select(age, gender, covid_deaths, excess_deaths, deaths)
  write_csv(d_merge, path = 'data/Spain/spain_all.csv')
}

# South Africa
process_sa_covid19 = function(){
  d_merge = read.csv('data/SouthAfrica/covid19_deaths_raw_mar.csv')
  ...
  write_csv(d_merge %>% select(-rate), path = 'data/SouthAfrica/covid19_deaths.csv')
}

# USA
process_usa = function(){
  tmp = readxl::read_xlsx('data/USA/Deaths-Age-Sex_Covid-19_USA_15.04.xlsx', sheet = 2)
  data = tmp[7:16, c(1,c(22, 24, 27))]
  data = data.frame(data)
  names(data) = c('age', 'male', 'female', 'both')
  data$male = as.numeric(data$male)
  ...
  #download.file(url, 'data/USA/usa_excess_mortality_all_mar.csv')
  data = read.csv('data/USA/usa_excess_mortality_all_mar.csv')
  ...
  write_csv(path = ("data/USA/excess_deaths.csv"),dataset)
  ...
  excess_death = excess_death %>%  
    filter((week >= 10 & year == "2020") | (week <= 13 & year == "2021")) %>%
    group_by(gender, week, age) %>%
  ...
  write_csv(path = paste0("data/USA/","usa_all.csv"),d_merge)
  ...
  write_csv(d_merge, path = 'data/USA/usa_all.csv')
}

# Zimbabwe
process_zimbabwe = function(){
  data = read_xlsx('data/Zimbabwe/zimbabwe_covid_raw.xlsx')
  data = data %>%
  ...
  data = data %>% select(-rate)
  names(data) <- c("age", "gender", "deaths")
  write_csv(data, file = 'data/Zimbabwe/covid19_deaths.csv')
}
