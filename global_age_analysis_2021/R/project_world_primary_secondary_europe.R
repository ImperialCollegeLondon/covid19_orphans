...
extrapolate_primary_secondary <- function(data = "covid"){
  ...
  if (data == "covid"){
    ...
    covid <- joined[joined$country %in% c("Pakistan", "Bangladesh", "Indonesia"),]
    covid <- select(covid, country, fitting_deaths)
  } else {
    joined <- readRDS("global_age_analysis_2021/data/tfr_covariates_excess.RDS")
    
    excess <- joined[joined$country %in% c("Pakistan", "Bangladesh", "Indonesia"),]
  ...
  print(output_ps$par)
  if (data == "excess"){
    saveRDS(output_ps$par, "global_age_analysis_2021/data/shiny/primary_secondary_coefficients_excess.RDS")
  } else {
  ...
  if (data == "excess"){
    saveRDS(ratio_dat, "global_age_analysis_2021/data/primary_secondary_ratios_excess.RDS")
  } else {
  ...
  if (data == "excess"){
    saveRDS(sprintf("%s [%s - %s]", 
            ...
            file = "global_age_analysis_2021/data/formatted_primary_secondary_excess.RDS")
  } else {
    saveRDS(sprintf("%s [%s - %s]", 
                    format(round(sum(joined$final_orphans), -2), big.mark = ",", trim = TRUE), 
                    format(round.choose(quantile(orphans_samples, probs = 0.025), 100, 0), big.mark = ",", trim = TRUE), 
  ...
  if (data == "excess"){
    saveRDS(orphans_sample, "global_age_analysis_2021/data/country_estimates_ps_excess.RDS")
  } else {
  ...
  if (data == "excess"){
    saveRDS(reg, "global_age_analysis_2021/data/age_outputs/reg_ps_excess.RDS")
  } else {
  ...
  if (data == "excess"){
    save(p_fit_ps_label, p_obs_pred_ps, p_loo_ps, file = "global_age_analysis_2021/data/extrapolate_primary_secondary_excess.RData")
  } else {
...
#extrapolate_primary_secondary(data = "excess")
extrapolate_primary_secondary(data = "covid")
