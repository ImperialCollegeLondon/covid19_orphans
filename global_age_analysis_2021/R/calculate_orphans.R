...
process_orphans_argentina = function(month = ""){
  ...
  write_csv(file = paste0('global_age_analysis_2021/data/Argentina/orphans_all.csv'), d_m1)
...
process_orphans_colombia = function(month = ""){
  ...
  d_m1 = d_m1%>% arrange(age)                  
  write_csv(file = paste0('global_age_analysis_2021/data/Colombia/orphans_all.csv'), d_m1)
  p <- ggplot(d_m1, aes(x = age, y = orphans, fill = gender)) +
    geom_bar(stat="identity", position=position_dodge()) +
    theme_bw()+
    theme(axis.text.x = element_text(angle = 30, hjust = 1, vjust = 1))+
  ...
  write_csv(file = 'global_age_analysis_2021/data/Colombia/all_data.csv', d_summary)
