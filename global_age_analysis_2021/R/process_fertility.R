...
process_fertility_plots = function(country){
  if (country == 'england_wales'){
    data_combine = read.csv('global_age_analysis_2021/data/UK/england_wales_fertility_all.csv')
    ...
    data_father = read.csv(paste0('global_age_analysis_2021/data/', country, '/', tolower(country), '_fertility_m_all.csv'))
    data_mother = read.csv(paste0('global_age_analysis_2021/data/', country, '/', tolower(country), '_fertility_f.csv'))
  ...
  ggsave(paste0("global_age_analysis_2021/figures/fertility_", tolower(country), ".pdf"), width = 10, height = 4)
}

# Process fertility data
process_fertility_all_countries = function(){
  all_combine = NaN
  list_file = list.files('global_age_analysis_2021/data/fertility_update', '.json')
  ...
  for (name_file in list_file)
  {
    ...
    json_data = suppressWarnings(fromJSON(paste(readLines(paste0('global_age_analysis_2021/data/fertility_update/',name_file)))))
    data = as.data.table(unlist(json_data))
    data$country = names(json_data)
    setnames(data, 1, 'idx')
    dataset = readxl::read_xls(paste0('global_age_analysis_2021/data/fertility_update/', strsplit(name_file, '.json')[[1]][1],'.xls'), sheet = 1)
...
process_england_wales_fertility = function(uncertainty = FALSE){
  ...
  # pop$gender = 'M'
  # saveRDS(pop1, "global_age_analysis_2021/data/UK/pop1_male.RDS")
  # saveRDS(pop, "global_age_analysis_2021/data/UK/pop_male.RDS")
  # 
  pop1 <- readRDS("global_age_analysis_2021/data/UK/pop1_male.RDS")
  ...
  pop <- readRDS("global_age_analysis_2021/data/UK/pop_male.RDS")
  ...
  # url = 'https://www.ons.gov.uk/file?uri=%2fpeoplepopulationandcommunity%2fbirthsdeathsandmarriages%2flivebirths%2fdatasets%2fbirthsbyparentscharacteristics%2f2018/parentscharacteristics2018workbook.xls'
  #download.file(url, 'global_age_analysis_2021/data/fertility/england_wales_births.xls')
  data = readxl::read_xls('global_age_analysis_2021/data/UK/england_wales_births.xls', sheet = 5)
  d_name = as.character(data[6,])
  d_name[1:2] = c('year', 'ignore')
  data = data[7:nrow(data),]
  names(data) = d_name
  ...
  # setnames(pop, 'sex', 'gender')
  # saveRDS(pop, "global_age_analysis_2021/data/UK/pop_female.RDS")
  
  pop = readRDS("global_age_analysis_2021/data/UK/pop_female.RDS")
...
process_france_fertility = function(uncertainty  = FALSE){
  ...
  data_pop = readxl::read_xlsx('global_age_analysis_2021/data/pop.xlsx', sheet = 2)
  countries = c('France')
  ...
  if (uncertainty == FALSE){
    ...
    #download.file(url, 'global_age_analysis_2021/data/afr.zip')
    #data = fread(unzip('global_age_analysis_2021/data/afr.zip', files = 'IHME_GBD_2019_FERTILITY_1950_2019_ASFR_Y2020M10D27.CSV'))
    data = read.csv("global_age_analysis_2021/data/IHME_GBD_2019_FERTILITY_1950_2019_ASFR_Y2020M10D27.CSV")
...
process_italy_fertility = function(uncertainty = FALSE){
  ...
  #data_combine = read.csv('global_age_analysis_2021/data/Italy/italy_fertility_m.csv')
  #data_combine$year = as.character(data_combine$year)
  #ggplot(data_combine) +
  #  geom_point(aes(x = age, y = fertility_rate, color = year)) +
  #  theme_bw()+
...
process_philippines_fertility = function(uncertainty = FALSE){
  ...
  # 
  # data_combine = read.csv('global_age_analysis_2021/data/Philippines/philippines_fertility_m.csv')
  # data_combine$year = as.character(data_combine$year)
  # ggplot(data_combine) +
  #   geom_point(aes(x = age, y = fertility_rate, color = year)) +
  #   theme_bw()+
...
process_spain_fertility = function(uncertainty = FALSE){
  ...
  # data_combine = read.csv('global_age_analysis_2021/data/Spain/spain_fertility_m.csv')
  # data_combine$year = as.character(data_combine$year)
  # ggplot(data_combine) +
  #   geom_point(aes(x = age, y = fertility_rate, color = year)) +
  #   theme_bw()+
...
process_usa_fertility = function(uncertainty = FALSE){
  ...
  # data_combine = read.csv('global_age_analysis_2021/data/USA/usa_fertility_m.csv')
  # data_combine$year = as.character(data_combine$year)
  # ggplot(data_combine) +
  #   geom_point(aes(x = age, y = fertility_rate, color = year)) +
  #   theme_bw()+
