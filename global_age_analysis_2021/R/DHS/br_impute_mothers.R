...
#-----------------------------------------------------------------------------
# Read in DHS data
all_data <- readRDS("data/DHS/brazil_mothers.RDS")
  ...
  for (j in 1:10){#10
    ...
    for (i in 1:length(mother_data$id_household)){
      ...
      if (is.na(mother_data$age_mother_now[i])){
        ...
        # If mother status is missing or unknown - work out if father should be alive
        if (mother_data$mother_status[i] > 3){
    ...
    mother_data <-  mother_data[which(mother_data$age_mother_birth < 50),]
    
    # select children born in last 5 year
    if (y == 1){
      selected_children <- mother_data[which(mother_data$age_child_now < 5),]
    } else {
      selected_children <- mother_data[which(mother_data$age_child_now >= 5*(y-1) & mother_data$age_child_now < 5*y),]
    }
    ...
    unique_women$age_months <- unique_women$age_mother_now*12 + (as.numeric(unique_women$birth_month_mother) - 1)
...
saveRDS(df_fertility, file = "data/Brazil/female_fertility.RDS")

p <- ggplot(df_fertility) + 
  geom_point(aes(ages, y2015, col = "2015")) + 
  geom_point(aes(ages, y2010, col = "2010")) +
  geom_point(aes(ages, y2005, col = "2005")) + 
  scale_color_manual(values = c("2015" = "black", "2010" = "blue", "2005" = "red")) + 
  theme_bw()
print(p)