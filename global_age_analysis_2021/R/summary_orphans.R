...
source("global_age_analysis_2021/R/colombia_excess.R")

combine_orphans <- function(country, grand_parents, month = ""){
  if (country == "EnglandWales"){
    data <- read.csv("global_age_analysis_2021/data/UK/england_wales_all_data.csv")
    ...
    data <- read.csv(paste0("global_age_analysis_2021/data/", country, "/all_data.csv"))
    names(data) = c('age', "gender", "excess", "covid19_deaths", "deaths", "nb_orphans")
    data$gender <- tolower(data$gender)
  } else {
    data <- read.csv(paste0("global_age_analysis_2021/data/", country, "/all_data.csv"))
