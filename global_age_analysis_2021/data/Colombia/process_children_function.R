...
process_children_all = function(country, is_child_mortality_needed, data_f){
  ...
  # Assume fertility rate over the age of 50 is 0 <- Need to check this.
  # Assume fertility_rate = value_e_rate / 1000 is the number of children born to each adult in one year
  # Assume fertility rate in 2020,2019 is same as 2018.
  ...
  write_csv(path = paste0('data/children/child_raw_', country,'_f.csv'), children)
  ...
  if (is_child_mortality_needed){
    child_m_matrix = read.csv(paste0('data/children/child_mortality_', country,'.csv'))
    child_and_m = as.matrix(children) * (1-as.matrix(child_m_matrix))
    child_and_m = as.data.frame(child_and_m)
    write_csv(path = paste0('data/children/child_all_', country, '_f.csv'), child_and_m)
  ...
  write_csv(path = paste0('data/children/child_all_list_', country, '_f.csv'), plot_c_and_m) 
  setnames(plot_c_and_m, 'mother_age', 'parents_age')
  plott = read.csv(paste0('data/children/child_all_list_', country, '_m.csv')) 
  setnames(plott, 'father_age', 'parents_age')
  plot_all = rbind(plot_c_and_m, plott)
  write_csv(path = paste0('data/children/child_all_list_', country, '_both_sex.csv'), plot_all) 
  ...
  ddf$gender = 'female'
  ddf$age = 1:100
  write_csv(path = paste0('data/children/', country,'_f.csv'), ddf)
  
  ddf = read.csv(paste0('data/children/', country,'_f.csv'))
  ddf_2 = read.csv(paste0('data/children/', country,'_m.csv'))
  ddf = rbind(ddf, ddf_2)
  p = ggplot(ddf) +
