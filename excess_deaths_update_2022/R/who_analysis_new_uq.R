...
d_country = d %>% filter(measure == "excess" & 
  ...
  mutate(sd = (as.numeric(uppr) - as.numeric(lwr))/(2*1.96)) %>%
  group_by(Country,`WHO region`) %>%
  summarise(mean = sum(as.numeric(mean)),
            sd = sqrt(sum(sd^2)))
...
names(d_country) = c("country", "region", "mean", "sd")
...
d_country_who_sd = d_country

lower = vector(length = length(d_country$country))
upper = vector(length = length(d_country$country))
means = vector(length = length(d_country$country))
for (i in 1:length(d_country$country)){
  x = rnorm(1000000, d_country$mean[i], d_country$sd[i])
  means[i] = mean(x)
  lower[i] = quantile(x, probs = 0.025)
  upper[i] = quantile(x, probs = 0.975)
}
d_country$total = means
d_country$lower = lower
d_country$upper = upper

d_country = select(d_country, country, region, mean, lower, upper)
...
d_country_who_sd <- d_country_who_sd[which(d_country_who_sd$mean > 0),]
who_deaths_country = left_join(d_country_who_sd, deaths_country, by = c("country"="Country.Region"))

lower = vector(length = length(who_deaths_country$country))
upper = vector(length = length(who_deaths_country$country))
means = vector(length = length(who_deaths_country$country))
for (i in 1:length(who_deaths_country$country)){
  x = rnorm(1000000, who_deaths_country$mean[i], who_deaths_country$sd[i])
  x = x * who_deaths_country$`X4.1.22`[i]/ who_deaths_country$`X12.31.21`[i] 
  means[i] = mean(x)
  if(!is.na(x[1])){
    lower[i] = quantile(x, probs = 0.025)
    upper[i] = quantile(x, probs = 0.975)
...
who_deaths_country$mean = means
who_deaths_country$lower = lower
who_deaths_country$upper = upper

# Remove Turkmenistan - no jhu data
who_deaths_country = who_deaths_country[which(who_deaths_country$country != "Turkmenistan"),] 


deaths_country = select(who_deaths_country, country, region, mean, lower, upper)
...
deaths_country_negative <- deaths_country_who[which(deaths_country_who$mean < 0),]
...
deaths_country =  deaths_country[which(deaths_country$mean > 0),]
...
for (i in 1:length(dates)){
  ...
  c_data <- combined_data[which(combined_data$date == dates[i]), 
                          c("country", "tfr", "tfr_l",  "tfr_u", "sd", "who_region", "europe", "mean", "lower", "upper")]
