...
calculate_all_orphans_time_series <- function(c_data, date, uncertainty, death_uncertainty, num_samples, source){
  # Read study data in
...
calculate_all_orphans_time_series_uq <- function(c_data, date, uncertainty, death_uncertainty, num_samples, source){
  # Read study data in
  study_ratios <- readRDS("excess_deaths_update_2022/data/calculated_ratios.RDS")
  
  # Calculate
  ps_orphans <- calculate_orphans_time_uq(c_data, readRDS("excess_deaths_update_2022/data/primary_secondary_coefficients.RDS"), 
                                       study_ratios[,c(1,4)], date, uncertainty = uncertainty, 
                                       death_uncertainty = death_uncertainty, num_samples, source,  type = "primary_secondary")
  primary_orphans <- calculate_orphans_time_uq(c_data, readRDS("excess_deaths_update_2022/data/primary_coefficients.RDS"), 
                                            study_ratios[,c(1,3)], date, uncertainty = uncertainty, 
                                            death_uncertainty = death_uncertainty, num_samples, source, type = "primary")
  parents_orphans <- calculate_orphans_time_uq(c_data, readRDS("excess_deaths_update_2022/data/parent_coefficients.RDS"), 
                                            study_ratios[,c(1,2)], date, uncertainty = uncertainty, 
                                            death_uncertainty = death_uncertainty,  num_samples, source, type = "orphanhood")
...
calculate_orphans_time_uq <- function(country_data, coeffs, study_ratios, date, 
                                   uncertainty, death_uncertainty, num_samples,
                                   source, type){
  set.seed(10)
  ...
  central <-  country_data$central_ratio * country_data$deaths_mean
  global_central = sum(central)
  ...
  if (uncertainty == FALSE){
    ...
    for (i in 1:n){
      ...
      if (death_uncertainty == TRUE){
        dn <- rnorm(length(country_data$country), mean = country_data$deaths_mean, sd = country_data$deaths_sd)
        estimated_orphans[, i] <- estimated_ratio[, i] * dn
        
      } else{
        estimated_orphans[, i] <- estimated_ratio[, i] * country_data$deaths_mean
      }
