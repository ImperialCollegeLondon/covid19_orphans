...
library(tidyverse)
library(readxl)
source("excess_deaths_update_2022/R/orphanhood_functions_uq.R")
...
d_country = select(d_country, country, region, mean, sd)
names(d_country) = c("country", "region", "deaths_mean", "deaths_sd")
...
who_deaths_country$new_mean = who_deaths_country$mean * who_deaths_country$`X4.1.22`/ who_deaths_country$`X12.31.21`
who_deaths_country$new_sd = who_deaths_country$sd * who_deaths_country$`X4.1.22`/ who_deaths_country$`X12.31.21`

# Remove Turkmenistan - no jhu data
who_deaths_country = who_deaths_country[which(who_deaths_country$country != "Turkmenistan"),] 


deaths_country = select(who_deaths_country, country, region, new_mean, new_sd)
names(deaths_country) = c("country", "region", "deaths_mean", "deaths_sd")
...
deaths_country_negative <- deaths_country_who[which(deaths_country_who$deaths_mean < 0),]
...
deaths_country =  deaths_country[which(deaths_country$deaths_mean > 0),]
...
for (i in 1:length(dates)){
  ...
  c_data <- combined_data[which(combined_data$date == dates[i]), 
                          c("country", "tfr", "tfr_l",  "tfr_u", "sd", "who_region", "europe", "deaths_mean", "deaths_sd")]
  names(c_data) <- c("country", "tfr", "tfr_l", "tfr_u", "sd",  "who_region", "europe", "deaths_mean",  "deaths_sd")
  orphans <- calculate_all_orphans_time_series_uq(c_data = c_data, date = dates[i], 
