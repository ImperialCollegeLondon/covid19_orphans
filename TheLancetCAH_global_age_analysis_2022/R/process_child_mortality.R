...
add_child_mortality = function(is_child_mortality_needed, country, uncertainty = FALSE){
  
  if (uncertainty == FALSE){
    children = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_raw_m.csv'))
    ...
    if (is_child_mortality_needed){
      ...
      child_m_matrix = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_mortality_rate.csv'))
      ...
      write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_m.csv'), child_and_m, row.names=FALSE)
      ...
      write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_list_m.csv'), plot_c_and_m, row.names=FALSE)
    } else{
    ...
    child_and_m = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_m.csv'))
    ddf = as.data.frame(apply(child_and_m, 1, sum))
    names(ddf) = 'children'
    ddf$gender = 'male'
    ddf$age = 1:100
    write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/children_m.csv'),ddf, row.names=FALSE)
    
  } else {
    
    children = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_raw_m_un.csv'))
    plot_c = as.data.frame(as.numeric(as.character(unlist(children))))
    plot_c$father_age = rep(1:100,18)
    plot_c$child_age =sort(rep(seq(18)-1, 100))
    setnames(plot_c, 1, 'prob')
    ...
    if (is_child_mortality_needed){
      ...
      child_and_m = as.matrix(children) * (1-as.matrix(child_m_matrix))
      child_and_m = as.data.frame(child_and_m)
      write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_m_un.csv'), child_and_m, row.names=FALSE)
      ...
      plot_c_and_m$gender = 'male'
      write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_list_m_un.csv'), plot_c_and_m, row.names=FALSE)
    } else{
      child_and_m = copy(children)
      child_and_m = as.data.frame(child_and_m)
      write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_m_un.csv'), child_and_m, row.names=FALSE)
    ...
    child_and_m = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_all_m_un.csv'))
    ddf = as.data.frame(apply(child_and_m, 1, sum))
    names(ddf) = 'children'
    ddf$gender = 'male'
    ddf$age = 1:100
    write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/children_m_un.csv'),ddf, row.names=FALSE)
  }
...
#   data = readxl::read_xlsx('TheLancetCAH_global_age_analysis_2022/data/NumberDeaths.xlsx', sheet = 2)
...
#   data_pop = readxl::read_xlsx('TheLancetCAH_global_age_analysis_2022/data/fertility_update/pop.xlsx', sheet = 2)
...
#   write.csv(d_merge, file = 'TheLancetCAH_global_age_analysis_2022/data/children/mortality_rate_all.csv', row.names=FALSE)
# }
...
process_child_mortality = function(country, countries){
  # rate per children 
  if (country != "Russia"){
    child = read.csv(paste0('TheLancetCAH_global_age_analysis_2022/data/child_mortality_rate', '.csv'))
    child = child %>% filter(country == countries)
    
  }else{
    child = read.csv('TheLancetCAH_global_age_analysis_2022/data/Russia/mortality_rate_all.csv')
  }
  ...
  write.csv(file = paste0('TheLancetCAH_global_age_analysis_2022/data/', country, '/child_mortality_rate.csv'), child_m_matrix, row.names=FALSE)
}  
...
process_children_mortality_england_wales = function(){
  ...
  #download.file(url, 'TheLancetCAH_global_age_analysis_2022/data/children/mortality_england_wales.xls')
  data = readxl::read_xls('TheLancetCAH_global_age_analysis_2022/data/UK/mortality_england_wales.xls', sheet = 5)
  nam <- as.character(data[6,])
  nam[16] <- "NA"
  names(data) = nam
  ...
  # saveRDS(pop,"TheLancetCAH_global_age_analysis_2022/data/UK/pop_child_mortality.RDS")
  
  pop  = readRDS("TheLancetCAH_global_age_analysis_2022/data/UK/pop_child_mortality.RDS")
  ...
  write.csv(data, file = 'TheLancetCAH_global_age_analysis_2022/data/UK/mortality_rate_england_wales.csv', row.names=FALSE)
}
...
process_infant_mortality_england_wales = function(){
  ...
  #download.file(url, 'TheLancetCAH_global_age_analysis_2022/data/UK/infant_england_wales.csv')
  data = read.csv('TheLancetCAH_global_age_analysis_2022/data/UK/infant_england_wales.csv')
  data  = data[7:nrow(data),]
  names(data) = c('year', 'mortality')
  data$age = '0 year'
  child_m = read.csv('TheLancetCAH_global_age_analysis_2022/data/UK/mortality_rate_england_wales.csv')
  ...
  write.csv(data, file = 'TheLancetCAH_global_age_analysis_2022/data/UK/mortality_rate_all_england_wales.csv', row.names=FALSE)
}

# Makes child mortality matrix for E and W
process_child_mortality_england_wales = function(){
  # rate 1e5 per children 
  child = read.csv('TheLancetCAH_global_age_analysis_2022/data/UK/mortality_rate_all_england_wales.csv')
  ...
  child_m_matrix = as.data.frame(child_m_matrix)
  names(child_m_matrix) = paste0(seq(0:17)-1, 'years')
  write.csv(file = 'TheLancetCAH_global_age_analysis_2022/data/UK/child_mortality_rate.csv', child_m_matrix, row.names=FALSE)
}  


process_child_mortality_russia = function(){
  data_04 = read.csv('TheLancetCAH_global_age_analysis_2022/data/Russia/russian_federation_0-4.csv')
  data_04 = data_04[,c(1,4:5)]
  setnames(data_04, 1:3, c('country', 'year', 'mortality'))
  data_04$age = '0-4'  
  data_59 = read.csv('TheLancetCAH_global_age_analysis_2022/data/Russia/russian_federation_5-9.csv')
  data_59 = data_59[,c(1,4:5)]
  setnames(data_59, 1:3, c('country', 'year', 'mortality'))
  data_59$age = '5-9'  
  data_1014 = read.csv('TheLancetCAH_global_age_analysis_2022/data/Russia/russian_federation_10-14.csv')
  data_1014 = data_1014[,c(1,4:5)]
  setnames(data_1014, 1:3, c('country', 'year', 'mortality'))
  data_1014$age = '10-14'  
  data_1519 = read.csv('TheLancetCAH_global_age_analysis_2022/data/Russia/russian_federation_15-19.csv')
  ...
  write.csv(data,file = paste0('TheLancetCAH_global_age_analysis_2022/data/children/mortality_rate_all_russain_federation.csv'), row.names=FALSE)
