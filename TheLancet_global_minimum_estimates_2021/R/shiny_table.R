...
calculate_all_orphans <- function(combined_data, date){
  # Read study data in
  study_ratios <- readRDS("TheLancet_global_minimum_estimates_2021/data/calculated_ratios.RDS")
  
  # Calculate
  ps_orphans <- calculate_orphans(combined_data, readRDS("TheLancet_global_minimum_estimates_2021/data/primary_secondary_coefficients.RDS"), 
                                  study_ratios[,c(1,4)])
  primary_orphans <- calculate_orphans(combined_data, readRDS("TheLancet_global_minimum_estimates_2021/data/primary_coefficients.RDS"), 
                                       study_ratios[,c(1,3)])
  parents_orphans <- calculate_orphans(combined_data, readRDS("TheLancet_global_minimum_estimates_2021/data/parent_coefficients.RDS"), 
                                       study_ratios[,c(1,2)])
...
calculate_all_orphans_time_series <- function(c_data, date){
  ...
  # Calculate
  ps_orphans <- calculate_orphans_time(c_data, readRDS("TheLancet_global_minimum_estimates_2021/data/primary_secondary_coefficients.RDS"), 
                                  study_ratios[,c(1,4)], date)
  primary_orphans <- calculate_orphans_time(c_data, readRDS("TheLancet_global_minimum_estimates_2021/data/primary_coefficients.RDS"), 
                                       study_ratios[,c(1,3)], date)
  parents_orphans <- calculate_orphans_time(c_data, readRDS("TheLancet_global_minimum_estimates_2021/data/parent_coefficients.RDS"), 
...
orphans_from_study <- function(country, date){
  deaths <- readRDS("TheLancet_global_minimum_estimates_2021/data/fitted_deaths.RDS")

  # Join study with tfr data
  data = readRDS("TheLancet_global_minimum_estimates_2021/data/tfr.RDS")
...
orphans_from_jhu <- function(country, date){
  ...
  # Read in JHU data
  jhu_url <- getURL(paste0("https://raw.githubusercontent.com/CSSEGISandTheLancet_global_minimum_estimates_2021/data/COVID-19/master/csse_covid_19_TheLancet_global_minimum_estimates_2021/data/csse_covid_19_daily_reports/", 
  ...
  multipliers <- read.csv('TheLancet_global_minimum_estimates_2021/data/multipliers.csv', header = FALSE)
...
orphans_from_own <- function(country, deaths, date){
  ...
  combined_data = readRDS("TheLancet_global_minimum_estimates_2021/data/tfr.RDS")
  combined_data$sd = (combined_data$tfr_u-combined_data$tfr_l)/(2*1.96)
