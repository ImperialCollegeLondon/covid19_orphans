...
for (y in 1:3){
  # Read in DHS data
  if (y != 3){
    all_data <- readRDS("data/DHS/ia_mothers_15.RDS")
  } else {
    all_data <- readRDS("data/DHS/ia_mothers_06.RDS")
  }
...
names(df_fertility) <- paste0("y", 2015 - 0:13)
df_fertility$ages <- factor(exposure_years$label, levels = c("under 15", labels))

saveRDS(df_fertility, file = "data/India/female_fertility.RDS")
...
p <- ggplot(df_fertility) + 
  ...
  scale_color_manual(values = c("2015" = "black", "2010" = "blue", "2006" = "red")) + 
  theme_bw()
print(p)