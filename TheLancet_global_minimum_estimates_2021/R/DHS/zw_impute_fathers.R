...
for (y in 1:3){
  # Read in DHS data
  if (y == 1){
    all_data <- readRDS("data/DHS/zw_fathers_15.RDS")
  } else if (y == 2){
    all_data <- readRDS("data/DHS/zw_fathers_10.RDS")
  } else {
    all_data <- readRDS("data/DHS/zw_fathers_05.RDS")
  }
...
saveRDS(df_fertility, file = "data/Zimbabwe/male_fertility.RDS")

p <- ggplot(df_fertility) + 
  geom_point(aes(ages, y2015, col = "2015")) + 
  geom_point(aes(ages, y2010, col = "2010")) +
  geom_point(aes(ages, y2004, col = "2005")) + 
