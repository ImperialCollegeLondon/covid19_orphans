...
for (y in 1:3){
  # Read in DHS data
  if (y == 1){
    all_data <- readRDS("data/DHS/co_fathers_15.RDS")
  } else if (y == 2){
    all_data <- readRDS("data/DHS/co_fathers_10.RDS")
  } else {
    all_data <- readRDS("data/DHS/co_fathers_05.RDS")
  }
...
names(df_fertility) <- paste0("y", 2015 - 0:14)
df_fertility$ages <- factor(exposure_years$label, levels = c("under 15", labels))
saveRDS(df_fertility, file = "data/Colombia/male_fertility.RDS")
