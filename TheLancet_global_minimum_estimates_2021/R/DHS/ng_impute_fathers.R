...
for (y in 1:4){
  # Read in DHS data
  if (y == 1){
    all_data <- readRDS("data/DHS/ng_fathers_18.RDS")
  } else if (y == 2){
    all_data <- readRDS("data/DHS/ng_fathers_13.RDS")
  } else if (y == 3){
    all_data <- readRDS("data/DHS/ng_fathers_08.RDS")
  } else {
    all_data <- readRDS("data/DHS/ng_fathers_03.RDS")
  }
  
  all_data <- select(all_data, -father_wi, -father_bmi, -father_smoking, -father_anemia, -household_smoking)
...
names(df_fertility) <- paste0("y", 2018 - 0:19)
df_fertility$ages <- factor(exposure_years$label, levels = c("under 15", labels))
saveRDS(df_fertility, file = "data/Nigeria/male_fertility.RDS")

p <- ggplot(df_fertility) + 
  geom_point(aes(ages, y2018, col = "2018")) + 
  geom_point(aes(ages, y2013, col = "2013")) +
  geom_point(aes(ages, y2008, col = "2008")) + 
  geom_point(aes(ages, y2003, col = "2003")) + 
  scale_color_manual(values = c("2018" = "black", "2013" = "blue", "2008" = "red", "2003" = "purple")) + 
  theme_bw()
print(p)
